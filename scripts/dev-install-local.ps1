...
function Invoke-Step {
  ...
  if ($DryRun) {
    ...
    Write-Output ($prefix + ($Command -join ' '))
    return
  }
  if ($WorkingDirectory) {
...
Write-Step 'BlueNote local developer install/link'
...
Invoke-Step -WorkingDirectory $BlueNoteDir -Command @('npm', 'link')
if ($Web) { Invoke-Step -WorkingDirectory $WebUiDir -Command @('npm', 'link') }
if ($Tui) { Invoke-Step -WorkingDirectory $TermPkgDir -Command @('bun', 'link') }

if ($DryRun -or (Get-Command bluenote -ErrorAction SilentlyContinue)) {
  Invoke-Step -Command @('bluenote', 'doctor')
} else {
  Write-Step 'bluenote is not on PATH yet; add npm/Bun global bins to PATH and run: bluenote doctor'
}
